...
const BlogPost = () => {
  ...
  if (loading) {
    return (
      <div className="p-4 md:p-6 bg-base-100 min-h-screen flex justify-center items-center">
    ...
    return (
      <div className="p-4 md:p-6 bg-base-100 min-h-screen">
        <div className="max-w-2xl mx-auto text-center">
          <h1 className="text-xl md:text-2xl font-bold mb-4">Post Not Found</h1>
  ...
  return (
    <div className="p-4 md:p-6 bg-base-100 min-h-screen">
      <div className="max-w-3xl mx-auto">
        <Link to="/blog" className="btn btn-ghost btn-sm mb-4 md:mb-6">
          ← Back to Blog
        </Link>
        ...
        <article className="prose prose-sm sm:prose lg:prose-xl max-w-none">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3 md:mb-4">{post.title}</h1>
          
          {post.type === 'user_review' && (
            <p className="text-base sm:text-lg text-base-content/70 mb-4 md:mb-6">Bike Model: {post.bikeModel}</p>
          )}
          
          <div className="flex items-center gap-3 mb-6 md:mb-8">
            <div className="avatar placeholder">
              <div className="bg-neutral-focus text-neutral-content rounded-full w-8 md:w-10">
                <span className="text-sm md:text-lg">{post.avatar}</span>
              </div>
            </div>
            <div>
              <p className="font-medium text-sm md:text-base">{post.author}</p>
              <p className="text-xs md:text-sm text-base-content/50">{new Date(post.date).toLocaleDateString()}</p>
            </div>
          </div>

          <div 
            className="blog-content mb-6 md:mb-8 prose prose-sm sm:prose-base max-w-none"
          ...
          <div className="flex flex-col-reverse sm:flex-row justify-between items-center gap-4">
            <Link to="/blog" className="btn btn-ghost w-full sm:w-auto">
              ← Back to all posts
            </Link>
            {post.type === 'admin_post' && (
              <button className="btn btn-primary w-full sm:w-auto">Share this post</button>
            )}
          </div>
        </article>
      </div>
